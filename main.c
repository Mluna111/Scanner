	...
	while ((currentCh = fgetc(src)) != EOF) {
		if (isspace(currentCh)) {
			...
			for (tempCh = fgetc(src); (isalnum(tempCh) || tempCh == '_');) {
				//build identifier lexeme____________________________________________
		...
		else if (currentCh == '(') {  // LPAREN, RPAREN, SEMICOLON, COMMA, ASSIGN, PLUS, MINUS, TIMES, DIV, SCAN_EOF
			return LPAREN;
		}
		else if (currentCh == ')') {
			return RPAREN;
...
int main(int argc, char* argv[])
{
	...
	int count = sizeof(mnemonic);
	enum tokenType enumber1 = ID;
